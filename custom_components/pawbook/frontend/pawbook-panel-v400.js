class PawBookPanelV400 extends HTMLElement {
  constructor() {
    super();
  ...
  weightChart(book) {
    const rows = [...(book.weights || [])].filter(x => x.date && Number.isFinite(Number(x.weight))).sort((a,b)=>String(a.date).localeCompare(String(b.date))).slice(-12);
  ...
  showAttachmentDialog() {
    ...
    const dialog = this.shadowRoot.querySelector("#dialog");
    dialog.innerHTML = `<div class="modal"><div class="modal-card"><div class="modal-head"><div><h2>Allegati</h2><p class="muted">Salva localmente referti, analisi e immagini. Max circa 2,5 MB per file.</p></div><button class="icon-btn" data-close>✕</button></div><label style="display:block;margin-top:16px"><span>File</span><input id="attachment-file" type="file" accept="image/*,application/pdf"></label><div class="modal-actions"><button class="secondary" data-close>Annulla</button><button id="save-attachment" disabled>Salva allegato</button></div></div></div>`;
    dialog.querySelectorAll("[data-close]").forEach(b=>b.addEventListener("click",()=>dialog.innerHTML=""));
    ...
    btn?.addEventListener("click", async ()=>{
      ...
      try { await this._hass.callWS({type:"pawbook/add_attachment",entry_id:book.entry_id,name:file.name,mime_type:file.type||"application/octet-stream",data:payload}); await this.loadBooks(); dialog.innerHTML=""; }
      ...
      <div class="page">
        <div class="topbar">
          <div class="brand">
            ...
            <div><h1>PawBook <span class="v2-badge">4.0</span></h1><div class="muted">Libretto sanitario digitale</div></div>
          </div>
        </div>
        ...
        <section class="home-overview" aria-label="Riepilogo PawBook 4.0">
          <button class="home-tile health" data-nav-target="smart-section">
            <span class="home-tile-icon">❤️</span><span><small>Stato di salute</small><strong>${smart.some(x=>x.level === "danger") ? "Da controllare" : smart.some(x=>x.level === "warn") ? "Attenzione" : "Tutto regolare"}</strong><em>${smart[0]?.text ? this.esc(smart[0].text) : "Nessun avviso"}</em></span>
          </button>
          <button class="home-tile vaccines" data-nav-target="vaccines-section">
            <span class="home-tile-icon">💉</span><span><small>Vaccini</small><strong>${vaccineGroups.length} tipi registrati</strong><em>${vaccineStatusCounts.expired ? `${vaccineStatusCounts.expired} scaduti` : vaccineStatusCounts.warning ? `${vaccineStatusCounts.warning} in scadenza` : "Nessuna scadenza critica"}</em></span>
          </button>
          <button class="home-tile visits" data-nav-target="visits-section">
            <span class="home-tile-icon">🩺</span><span><small>Visite</small><strong>${lastVisit ? this.formatDate(lastVisit.date) : "Nessuna visita"}</strong><em>${book.visits?.length || 0} registrazioni</em></span>
          </button>
          <button class="home-tile genealogy" data-nav-target="genealogy-section">
            <span class="home-tile-icon">🌳</span><span><small>Genealogia</small><strong>${book.genealogy && Object.keys(book.genealogy).length ? "Pedigree disponibile" : "Non importata"}</strong><em>ENCI e salute genealogica</em></span>
          </button>
        </section>
        ...
        <section class="grid">
          ...
          <article class="card wide vaccines-dashboard" id="vaccines-section">
            <div class="card-head"><div><h3>💉 Vaccinazioni</h3><small class="muted">Cronologia completa raggruppata per vaccino</small></div><button class="small-btn" data-form="vaccination">Aggiungi</button></div>
            ...
            ${vaccineGroups.length ? `<div class="vaccine-groups">${vaccineGroups.map(group => {
              ...
              return `<details class="vaccine-group ${status.key}" ${group === vaccineGroups[0] ? "open" : ""}>
                <summary>
                  <span class="vaccine-group-title"><span class="vaccine-dot">${status.icon}</span><span><strong>${this.esc(group.name)}</strong><small>${status.label}</small></span></span>
...
if (!customElements.get("pawbook-panel-v400")) {
  customElements.define("pawbook-panel-v400", PawBookPanelV400);
}
