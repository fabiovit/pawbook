      ...
      <div class="page">
        <div class="topbar">
          <div class="brand">
            ...
            <div><h1>PawBook <span class="v2-badge">4.8.0</span></h1><div class="muted">Libretto sanitario digitale</div></div>
          </div>
        </div>
        ...
        <section class="smart-dashboard" aria-label="Smart Dashboard PawBook">
          ...
          <div class="smart-reminder-strip">
            <span>🔔 Promemoria</span>
            <strong>${nextVax && this.vaccinationStatus({next:nextVax}).days <= 30 ? `Vaccino ${this.esc(nextVax.name || "")} in scadenza` : treatmentStats.active.some(item=>item.ends_on && new Date(`${item.ends_on}T00:00:00`) - new Date() <= 3*86400000) ? "Una terapia termina entro 3 giorni" : heatStats.forecast && heatStats.forecast.from <= new Date().toISOString().slice(0,10) && heatStats.forecast.to >= new Date().toISOString().slice(0,10) ? "Siamo nella finestra stimata del prossimo calore" : "Nessun promemoria urgente"}</strong>
            <button type="button" class="small-btn secondary" data-nav-target="calendar-section">Apri calendario</button>
          </div>
...
if (!customElements.get("pawbook-panel-v480")) {
  customElements.define("pawbook-panel-v480", PawBookPanelV420);
}
