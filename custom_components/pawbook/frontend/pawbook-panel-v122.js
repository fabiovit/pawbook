class PawBookPanelV122 extends HTMLElement {
  constructor() {
    super();
  ...
  render() {
    ...
    const styles = `
      ...
      * { box-sizing: border-box; }
      ...
      .dashboard-nav { display:grid; grid-template-columns:repeat(7,minmax(0,1fr)); gap:4px; margin:4px 0 24px; padding:14px 12px 8px; border-radius:24px; background:var(--card-background-color); box-shadow:var(--ha-card-box-shadow,0 2px 12px rgba(0,0,0,.06)); }
      ...
      @media(max-width:900px) {
        ...
        .pet-photo,.photo-button .placeholder { width:100px; height:100px; border-radius:20px; }
        .identity h2 { font-size:28px; }
        .identity-grid { font-size:13px; gap:7px 12px; }
        .stats { grid-template-columns:1fr; }
        .dashboard-nav { grid-template-columns:repeat(4,minmax(0,1fr)); overflow-x:auto; }
        .dashboard-nav button { min-height:66px; font-size:12px; }
    ...
    this.shadowRoot.innerHTML = `
      ...
      <div class="page">
        ...
        <nav class="dashboard-nav" aria-label="Sezioni PawBook">
          ...
          <button data-nav-action="settings"><span class="nav-icon">⚙</span><span>Impostazioni</span></button>
        </nav>
...
if (!customElements.get("pawbook-panel-v122")) {
  customElements.define("pawbook-panel-v122", PawBookPanelV122);
}
