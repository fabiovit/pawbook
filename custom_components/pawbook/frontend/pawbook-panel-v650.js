...
await import(BASE_PANEL_URL);

const PawBookPanelV640 = customElements.get("pawbook-panel-v640");

if (!PawBookPanelV640) {
  throw new Error("PawBook 6.5: base panel v640 non disponibile");
}

class PawBookPanelV650 extends PawBookPanelV640 {
  applyActiveView() {
  ...
  _applyVisualPolish650() {
    const root = this.shadowRoot;
    if (!root || root.querySelector("style[data-pawbook-650]")) return;

    const style = document.createElement("style");
    style.dataset.pawbook650 = "true";
    style.textContent = `
      .page {
      ...
      /* Elimina le vecchie bande azzurre verticali. */
      ...
      /* Più respiro: niente titoli incollati al bordo. */
      .grid > article {
      ...
      /* Menu principale: poche destinazioni chiare. */
      ...
      /* Smart Health: righe più ariose, senza la vecchia banda appiccicata. */
...
if (!customElements.get("pawbook-panel-v650")) {
